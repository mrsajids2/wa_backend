...
const { sendOTPwithFallback, signupSendOTP } = require("../utils/verification");
...
exports.signUpCompany = async (req, res, next) => {
  ...
  try {
    ...
    let existingCompany;
    if (mobile) {
      existingCompany = await Company.findByMobile("masters", mobile);
    } else if (email) {
      existingCompany = await Company.findByEmail("masters", email);
    }
    if (existingCompany) {
...
exports.vefifySignUpOtpnSignup = async (req, res, next) => {
  ...
  try {
    ...
    if (Number(session.session.otp) !== Number(otp)) {
      ...
      // Step 4: OTP is valid, proceed with registration
      const companyresponse = await Company.create("masters", req.body);
      if (!companyresponse) {
        return response.serverError(res);
      }
...
exports.forgetPasswordSendOTP = async (req, res, next) => {
  ...
  try {
    ...
    if (existingCompany) {
      ...
      // Step 4: Sending OTP to whatsapp
      const mobilewithcountrycode = existingCompany.countrycode + mobile;
      console.log(
        mobilewithcountrycode,
        otp,
        existingCompany.emailid,
...
exports.forgetPasswordReset = async (req, res, next) => {
  ...
  try {
    ...
    if (existingCompany) {
      ...
      const updated = await Company.updatePassword(
        "masters",
        password,
        existingCompany
      );
